    ...
    do{
        ...
        case 1:
            if(!colaVacia(*cola)){
                ...
                scanf("%d", &aux);
                char *nameaux = borrarArticulo(cola, &aux);
                printf("\n%s elimidado del carrito.\n", nameaux);
            }else{
...
void agregarCarrito(Cola *cola, Articulo articulo, int cantidad){
    ...
    insertar(cola, articuloCarrito); // Se agrega el producto al carrito
...
void listarArticulos(int a, Articulo *art){
    a--;
    ...
    while(fgets(linea, sizeof(linea), archivo)){
        ...
        art[(i - 1)].nombre = strdup(fnombre);
        art[(i - 1)].marca = strdup(fmarca);
        art[(i - 1)].precio = fprecio;
        art[(i - 1)].stock = fstock;
        art[(i - 1)].ventas = fventas;
...
void explorar(Cola *cola){
    ...
    int n = 20;
    Articulo *art = (Articulo *)calloc(n, sizeof(Articulo)); // Arreglo para almacenar los productos
    do{
        printf("\n---- EXPLORAR ----\n1) Categorías\n2) Estadísticas\n3) Regresar\nIngrese opción: ");
        scanf("%d", &opcion);
        switch(opcion){
        case 1:
            ...
            if(aux != 0){
                ...
                while(cantidad > art[aux - 1].stock){
                    printf("No hay suficiente stock. Seleccione otra cantidad: ");
                    scanf("%d", &cantidad);
                }
                agregarCarrito(cola, art[aux - 1], cantidad);
            }
            break;
        case 2:
            estadisticas();
